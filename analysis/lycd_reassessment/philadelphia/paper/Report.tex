...
\begin{table}[t]
  ...
  every surface; the raw figure is the same statistic on every row. The extraction is rebuilt
  without its published calibration and re-levelled inside each fold, for the reason given in
  the text.}
  \label{tab:heldout}
...
The fifth candidate is worth its own sentence, because it is the one an appraiser would
reach for first. Extraction --- decomposing improved sales into land and structure and taking
land as the residual --- is a recognized method under Section~3(d), and this repository already
runs one (a semi-parametric estimator that differences out location). As published it is
re-levelled against the same vacant sales this study scores on, so its apparent accuracy is
in-sample and cannot be compared with anything. Rebuilt without that step and re-levelled
inside each fold instead, it scores \LycdSfourcvTrCod{} --- worse than OPA's
\LycdSzeroTrCod{} by \LycdSfourcvVsSzeroDiffAbs{} COD, and worse than the interpolation by
\LycdStwoVsSfourcvDiffAbs{} (\LycdStwoVsSfourcvLo{} to \LycdStwoVsSfourcvHi{}). The
extraction carries real land signal --- its median ratio is \LycdSfourcvTrMedian{}, so it is
not mis-levelled --- but it is noisier about where land is valuable than the land sales are
directly. On a method the ordinance explicitly contemplates, that is worth knowing: the
residual of an improved sale is a weaker instrument than a vacant one, and Philadelphia has
enough of the latter to prefer them.
